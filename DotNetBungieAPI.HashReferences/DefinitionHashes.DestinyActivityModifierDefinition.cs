namespace DotNetBungieAPI.HashReferences
{
	public static partial class DefinitionHashes
	{
		public static class ActivityModifiers
		{
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint DeviousThievery = 2159250954;
			/// <summary>
			/// 	Enemies throw grenades significantly more often.
			/// </summary>
			public const uint ScorchedEarth = 703904464;
			/// <summary>
			/// 	[Arc] Arc Shields
			/// </summary>
			public const uint ShieldedFoes_3171609188 = 3171609188;
			/// <summary>
			/// 	Scorn Raiders now have Void shields.
			/// </summary>
			public const uint RaiderShield = 3566681029;
			/// <summary>
			/// 	Capture the zones and defeat your opponents for points. The team with the most points wins.
			/// <para/>
			/// 	Zone Advantage: The state when your team owns 2 zones. Your team gains +2 points for every Guardian you defeat while in this state.
			/// <para/>
			/// 	Power Play: The state when your team owns all 3 zones. Your team gains +3 points for every Guardian you defeat while in this state.
			/// <para/>
			/// 	Capture Participant: You gain an additional +1 point for each player that participated in the zone capture.
			/// <para/>
			/// </summary>
			public const uint ControlRules = 2201679031;
			/// <summary>
			/// 	Limited fireteam revives. Gain additional revives by defeating Champions.
			/// </summary>
			public const uint LimitedRevives_4239965093 = 4239965093;
			/// <summary>
			/// 	When defeated, Psions spawn Void grenades at their feet.
			/// </summary>
			public const uint Pestilence = 4230548176;
			/// <summary>
			/// 	"Salvation's Edge" raid challenge modes.
			/// </summary>
			public const uint BalancedDietChallenge = 1553052482;
			/// <summary>
			/// 	Round 1: Cabal
			/// <para/>
			/// 	Round 2: Taken
			/// <para/>
			/// 	Final Round: Crota
			/// <para/>
			/// </summary>
			public const uint ContestantsTheWayoftheHoof_2675177792 = 2675177792;
			/// <summary>
			/// 	{var:4005007457}% increase to incoming Strand damage.
			/// </summary>
			public const uint StrandThreat = 1598472557;
			/// <summary>
			/// 	Complete placement matches to determine your starting position on the current Season's Ranked ladder.
			/// <para/>
			/// 	Matches played: {var:4027254163} of {var:2701182729}
			/// <para/>
			/// </summary>
			public const uint PlacementSeries = 1505377421;
			/// <summary>
			/// 	To win, score more points that your opponent by capturing and defending the zones. The Hunt activates twice per match.
			/// <para/>
			/// 	Zones: Gain 2 points for each zone your team controls at each time interval.
			/// <para/>
			/// 	Zone Advantage: The state when your team controls 2 zones. Your team gains 4 points at each time interval.
			/// <para/>
			/// 	Power Play: The state when your team controls all 3 zones. Your team gains 6 points at each time interval.
			/// <para/>
			/// 	The Hunt: Occurs twice per match. Caiatl sends a high-value zone via Drop Pod. This zone is quick to capture and awards a large number of points over a shorter period of time.
			/// <para/>
			/// </summary>
			public const uint IronBannerFortress = 3666600199;
			/// <summary>
			/// 	You will face combatants with [Void] Void and Strand shields.
			/// </summary>
			public const uint ShieldedFoes_1444852954 = 1444852954;
			/// <summary>
			/// 	Power is disabled in this activity. Combatant difficulty increases with each set.
			/// </summary>
			public const uint EscalatingDifficulty_1545596619 = 1545596619;
			/// <summary>
			/// 	When you and your ally are reunited, your weapon handling and reload speed are greatly increased, and you gain increased flinch resistance.
			/// <para/>
			/// 	When you and your ally are drifting apart, your sprint and slide abilities are enhanced.
			/// <para/>
			/// </summary>
			public const uint DoublesRules = 4093484583;
			/// <summary>
			/// 	Taken combatants generate blight geysers when defeated.
			/// </summary>
			public const uint Epitaph = 998275325;
			/// <summary>
			/// 	{var:2189146210}% bonus to outgoing Arc damage.
			/// </summary>
			public const uint ArcSurge = 2691200658;
			/// <summary>
			/// 	Matchmaking prefers to avoid players with widely disparate skills.
			/// </summary>
			public const uint Matchmaking_1345228604 = 1345228604;
			/// <summary>
			/// 	Stasis damage increases slightly from all sources.
			/// </summary>
			public const uint StasisSinge = 2432720046;
			/// <summary>
			/// 	{var:1027206613}% bonus to Glaive damage.
			/// </summary>
			public const uint OverchargedGlaive = 2743796883;
			/// <summary>
			/// 	Keep it secret.  Keep it safe.
			/// </summary>
			public const uint Classified_4292382471 = 4292382471;
			/// <summary>
			/// 	You will face [Stagger] Unstoppable Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_2475764450 = 2475764450;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:219516062}
			/// </summary>
			public const uint ExoticDropRateBoostsGained_792361634 = 792361634;
			/// <summary>
			/// 	A Siegehook Ogre awaits you on wave 50.
			/// </summary>
			public const uint BossRotation_1894842396 = 1894842396;
			/// <summary>
			/// 	{var:4005007457}% increase to incoming Solar damage.
			/// </summary>
			public const uint SolarThreat = 3517267764;
			/// <summary>
			/// 	To win, your team must become the last Guardian(s) standing by defeating the entire opposing team or capturing the zone.
			/// <para/>
			/// 	Dominion: A capture zone appears at the start of each round. Win the round by capturing the zone.
			/// <para/>
			/// 	Elimination: Wins can also be achieved by defeating the opposing team.
			/// <para/>
			/// 	Revives: Teammate revives are enabled.
			/// <para/>
			/// </summary>
			public const uint DominionRules = 4263658911;
			/// <summary>
			/// 	"Salvation's Edge" raid challenge modes.
			/// </summary>
			public const uint VariedGeometryChallenge = 2821522136;
			/// <summary>
			/// 	To win, defeat your opponents, collect their crests, and deposit them at Cabal beacons.
			/// <para/>
			/// 	Cabal Beacons: Beacons appear at various locations on the map, two at a time. They have limited duration before they switch to another position. Collect crests and deposit them at beacons for points.
			/// <para/>
			/// 	The Hunt: Activates when the score threshold is met. This is a limited, special state when Caiatl sends a high-value beacon protected with turrets. The high-value beacon awards a bonus score for each crest deposited.
			/// <para/>
			/// </summary>
			public const uint IronBannerTribute = 3456379103;
			/// <summary>
			/// 	Defeated Guardians drop a crest. Secure opposing crests to score points. Secure allied crests to deny your opponents points.
			/// </summary>
			public const uint SupremacyRules = 3296644900;
			/// <summary>
			/// 	This activity does not scale combatant difficulty according to fireteam size.
			/// </summary>
			public const uint BalancedScales = 1264329482;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint CrossfireChallenge = 138641527;
			/// <summary>
			/// 	{var:4005007457}% increase to incoming Arc damage.
			/// </summary>
			public const uint ArcThreat = 186409259;
			/// <summary>
			/// 	All Trials Rank point gains are increased!
			/// </summary>
			public const uint IncreasedTrialsRank = 1361609633;
			/// <summary>
			/// 	Keep it secret.  Keep it safe.
			/// </summary>
			public const uint Classified_1305687814 = 1305687814;
			/// <summary>
			/// 	If the timer reaches 0:00, the activity will restart.
			/// </summary>
			public const uint ForcedRebootTimer = 4134002479;
			/// <summary>
			/// 	All challenges are active.
			/// </summary>
			public const uint RaidChallenges_2323293470 = 2323293470;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_3283187796 = 3283187796;
			/// <summary>
			/// 	Activates on wave 31.
			/// <para/>
			/// 	If your fireteam falls, everyone is returned to orbit.
			/// <para/>
			/// </summary>
			public const uint DelayedExtinguish_2787320573 = 2787320573;
			/// <summary>
			/// 	All Vanguard Rank point gains are increased!
			/// </summary>
			public const uint IncreasedVanguardRank = 745014575;
			/// <summary>
			/// 	Power weapons deal more damage, and more ammo is available.
			/// </summary>
			public const uint Heavyweight = 1681061669;
			/// <summary>
			/// 	Extra Champions
			/// <para/>
			/// 	Locked Loadout
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint MasterModifiers_501815068 = 501815068;
			/// <summary>
			/// 	Matchmaking does not utilize skill.
			/// </summary>
			public const uint Matchmaking_3206202148 = 3206202148;
			/// <summary>
			/// 	Round 1: Taken
			/// <para/>
			/// 	Round 2: Cabal
			/// <para/>
			/// 	Final Round: Zydron
			/// <para/>
			/// </summary>
			public const uint ContestantsDisciplesoftheNeigh_1902839616 = 1902839616;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint TheOnlyOracleForYou = 274420331;
			/// <summary>
			/// 	Ability recharge rate is increased.
			/// </summary>
			public const uint Frontliner = 2378419670;
			/// <summary>
			/// 	Extra Champions
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint MasterModifiers_4087563963 = 4087563963;
			/// <summary>
			/// 	Defeated Guardians will respawn after a short time.
			/// </summary>
			public const uint RespawnsEnabled = 3450476141;
			/// <summary>
			/// 	Exploding units have more health.
			/// </summary>
			public const uint Martyr_3398790191 = 3398790191;
			/// <summary>
			/// 	You will face [Disruption] Overload and [Stagger] Unstoppable Champions.
			/// </summary>
			public const uint ChampionFoes_40182179 = 40182179;
			/// <summary>
			/// 	Potential Data buffs:
			/// <para/>
			/// 	• Chance to spawn an Orb of Power on combatant final blows
			/// <para/>
			/// 	• Chance to spawn a Stasis Crystal on combatant final blows
			/// <para/>
			/// 	• Chance to gain class ability energy combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_2587905069 = 2587905069;
			/// <summary>
			/// 	{var:1027206613}% bonus to Bow damage.
			/// </summary>
			public const uint OverchargedBow = 929044687;
			/// <summary>
			/// 	Defend and repair the ADU against waves of combatants. If the ADU is destroyed, the activity will end.
			/// <para/>
			/// 	Scraps: Activity currency used to purchase and upgrade defenses at the start of every set and after waves 3 and 6.
			/// <para/>
			/// 	Rewards: Received after defeating the boss wave.
			/// <para/>
			/// 	Bonus Objectives: These reward resources and armaments.
			/// <para/>
			/// </summary>
			public const uint OnslaughtRules_612342440 = 612342440;
			/// <summary>
			/// 	Regeneration is greatly impaired. Defeating enemies may create wells of Light.
			/// </summary>
			public const uint Attrition = 3346897368;
			/// <summary>
			/// 	Enhanced radar. Take increased damage from melee.
			/// </summary>
			public const uint Empath_2208550513 = 2208550513;
			/// <summary>
			/// 	Capture the zones and defeat your opponents for points. The team with the most points wins. This is a faster version of Control.
			/// <para/>
			/// 	Momentum: You can capture zones faster. Weapon damage is significantly increased. Abilities and Supers regenerate when you defeat an opponent. Passive regeneration is significantly decreased. 
			/// <para/>
			/// 	Zone Advantage: The state when your team owns 2 zones. Your team gains +2 points for every Guardian you defeat while in this state.
			/// <para/>
			/// 	Power Play: The state when your team owns all 3 zones. Your team gains +3 points for every Guardian you defeat while in this state.
			/// <para/>
			/// </summary>
			public const uint MomentumControlRules = 2327064309;
			/// <summary>
			/// 	Matchmaking prefers players with similar Competitive Rank.
			/// </summary>
			public const uint Matchmaking_2266489848 = 2266489848;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint OutofItsWay = 2814846409;
			/// <summary>
			/// 	Your Power is significantly below the recommended level for this activity.
			/// </summary>
			public const uint Brutal = 1108831612;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint ForeverFight = 3632655552;
			/// <summary>
			/// 	Defeat combatants to generate snowballs.
			/// </summary>
			public const uint Snowstorm = 2483219497;
			/// <summary>
			/// 	When defeated, Goblins spawn Arc pools that causes damage over time.
			/// </summary>
			public const uint Shocker = 321244196;
			/// <summary>
			/// 	Melee abilities deal more damage and recharge much faster.
			/// </summary>
			public const uint DaodanSurge = 3787716856;
			/// <summary>
			/// 	Keep it secret.  Keep it safe.
			/// </summary>
			public const uint Classified_1555213579 = 1555213579;
			/// <summary>
			/// 	Ability recharge time: Slowed
			/// <para/>
			/// 	Special ammo is earned by engaging in combat.
			/// <para/>
			/// 	Special ammo is not lost on death.
			/// <para/>
			/// </summary>
			public const uint CheckmateModifiers = 2001067135;
			/// <summary>
			/// 	Win the match by winning more rounds than your opponents. Defeat them to score points; the team with the most points wins a round.
			/// </summary>
			public const uint ShowdownRules = 3388053151;
			/// <summary>
			/// 	Dealing damage with Scout Rifles, Pulse Rifles, and Grenade Launchers gradually reduces their damage while increasing the damage of Submachine Guns and Shotguns, and vice versa. 
			/// <para/>
			/// 	Other weapon types deal significantly less damage.
			/// <para/>
			/// </summary>
			public const uint Oscillation = 38299602;
			/// <summary>
			/// 	Super recharge rate is increased.
			/// </summary>
			public const uint SurgeProtector = 2742259563;
			/// <summary>
			/// 	You will face combatants with [Arc] Arc and [Solar] Solar shields.
			/// <para/>
			/// 	Activity modifiers may add more shield types.
			/// <para/>
			/// </summary>
			public const uint ShieldedFoes_3230561446 = 3230561446;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint IlluminatedTormentChallenge = 766975360;
			/// <summary>
			/// 	{var:2189146210}% bonus to outgoing Void damage.
			/// </summary>
			public const uint VoidSurge = 3196075844;
			/// <summary>
			/// 	All challenges are active.
			/// </summary>
			public const uint RaidChallenges_2596648371 = 2596648371;
			/// <summary>
			/// 	Combatant splash damage is increased.
			/// <para/>
			/// 	Activates Immediately.
			/// <para/>
			/// </summary>
			public const uint Blasts = 1410027362;
			/// <summary>
			/// 	All weapons are disabled! Abilities are your only defense.
			/// <para/>
			/// 	Your abilities recharge much faster, and your camera is set to third person.
			/// <para/>
			/// </summary>
			public const uint ParacausalFocus = 1345530799;
			/// <summary>
			/// 	Dealing damage with a Solar weapon now applies scorch to your targets, but your foes also now apply scorch to you when they deal Solar or explosive damage.
			/// </summary>
			public const uint AshestoAshes = 158678773;
			/// <summary>
			/// 	To win, be the first team to summon and defeat your Primeval. 
			/// <para/>
			/// 	Motes: Defeated foes will drop Motes you can pick up. Watch out! If you die, you'll lose any Motes you're holding. 
			/// <para/>
			/// 	Bank: Deposit Motes you're holding in the bank. Bank enough Motes, and you'll summon a Primeval. 
			/// <para/>
			/// 	Blockers: Bank enough Motes at once to summon Blockers that lock the opposing team's bank. 
			/// <para/>
			/// 	Invasions: As your team banks Motes, an invasion portal will open, allowing you to enter the opposing team's area. Take them out to make them drop their Motes!
			/// <para/>
			/// </summary>
			public const uint GambitRules = 2841995557;
			/// <summary>
			/// 	Enemy melee attacks are significantly more powerful, and radar is disabled.
			/// </summary>
			public const uint Blackout = 1427917745;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_2203953934 = 2203953934;
			/// <summary>
			/// 	[Solar] Solar Shields
			/// </summary>
			public const uint ShieldedFoes_2965677044 = 2965677044;
			/// <summary>
			/// 	[Arc] Arc, [Solar] Solar, and [Void] Void Shields
			/// </summary>
			public const uint ShieldedFoes_2288210988 = 2288210988;
			/// <summary>
			/// 	All challenges are active.
			/// </summary>
			public const uint RaidChallenges_1789323317 = 1789323317;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:1510191578}
			/// </summary>
			public const uint ExoticDropRateBoostsGained_582228082 = 582228082;
			/// <summary>
			/// 	Rank-and-file combatants deal increased damage.
			/// </summary>
			public const uint RankandFileDamageIncrease = 77928345;
			/// <summary>
			/// 	Potential Data buffs:
			/// <para/>
			/// 	• Spawn an ammo crate
			/// <para/>
			/// 	• Chance to spawn Threadlings on combatant final blows
			/// <para/>
			/// 	• Chance to gain grenade energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_3620773775 = 3620773775;
			/// <summary>
			/// 	Depositing data will award one of the following buffs:
			/// <para/>
			/// 	• Chance to spawn an Orb of Power on combatant final blows
			/// <para/>
			/// 	• Chance to Jolt on Arc Damage
			/// <para/>
			/// 	• Chance to gain melee energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_4269081391 = 4269081391;
			/// <summary>
			/// 	Being hit by a melee attack slows you.
			/// </summary>
			public const uint ChillTouch = 1572913484;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint TheCoreFour = 191124900;
			/// <summary>
			/// 	This week, a Subjugator Omen awaits you on wave 50.
			/// </summary>
			public const uint WeeklyBoss_65390994 = 65390994;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint WaitForIt = 3135466059;
			/// <summary>
			/// 	This playlist contains Collision and Clash. When you launch, it will randomly select one of the modes for you to play.
			/// <para/>
			/// 	Collision: To win, score more points than your opponents by capturing and controlling the zone. There will be one zone active at a time. Defeating your opponents does not directly grant points.
			/// <para/>
			/// 	Clash: To win, defeat your opponents to score more points than them!
			/// <para/>
			/// </summary>
			public const uint CompetitiveDivisionRules = 732136790;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint DefensesDown = 2255984700;
			/// <summary>
			/// 	{var:1027206613}% bonus to damage for all Grenade Launchers.
			/// </summary>
			public const uint OverchargedGrenadeLauncher = 3758645512;
			/// <summary>
			/// 	Exploder units have more health.
			/// </summary>
			public const uint Martyr_1553444357 = 1553444357;
			/// <summary>
			/// 	"Salvation's Edge" raid challenge modes.
			/// </summary>
			public const uint AtCapacityChallenge = 1204802854;
			/// <summary>
			/// 	[Void] Void and Strand Shields
			/// </summary>
			public const uint ShieldedFoes_2524382624 = 2524382624;
			/// <summary>
			/// 	Spawn Cadence: Heavy ammo will spawn every round after a long duration.
			/// </summary>
			public const uint HeavyAmmoRulesAlpha = 48917747;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint CosmicEquilibriumChallenge = 2070303007;
			/// <summary>
			/// 	Locked Loadout
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint ExpertModifiers_1139702033 = 1139702033;
			/// <summary>
			/// 	When defeated, challenging combatants drop fire at their location.
			/// </summary>
			public const uint HotStep = 1073070726;
			/// <summary>
			/// 	{var:1027206613}% bonus to Scout Rifle damage.
			/// </summary>
			public const uint OverchargedScoutRifle = 214700178;
			/// <summary>
			/// 	{var:1027206613}% bonus to Linear Fusion Rifle damage.
			/// </summary>
			public const uint OverchargedLinearFusionRifle = 3320777106;
			/// <summary>
			/// 	This week, an Elder Revenant awaits you on wave 50.
			/// </summary>
			public const uint WeeklyBoss_2759838796 = 2759838796;
			/// <summary>
			/// 	Increased respawn times.
			/// </summary>
			public const uint LongerRespawns = 3224024504;
			/// <summary>
			/// 	[Solar] Solar and [Void] Void Shields
			/// </summary>
			public const uint ShieldedFoes_1377274412 = 1377274412;
			/// <summary>
			/// 	Stasis final blows grant melee and grenade energy. Solar subclasses receive a lot; other subclasses receive a little.
			/// </summary>
			public const uint StasisThermalSwap = 2072906693;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_4047923249 = 4047923249;
			/// <summary>
			/// 	To win, score more points than your opponent by capturing and controlling zones.
			/// <para/>
			/// 	Zones: Score points for each zone your team controls.
			/// <para/>
			/// </summary>
			public const uint ZoneControlRules = 3520387869;
			/// <summary>
			/// 	Round 1: Hive
			/// <para/>
			/// 	Round 2: Vex
			/// <para/>
			/// 	Final Round: Valus Ta'aurc
			/// <para/>
			/// </summary>
			public const uint ContestantsDisciplesoftheNeigh_3899751486 = 3899751486;
			/// <summary>
			/// 	Combatants are granted random Banes.
			/// </summary>
			public const uint RandomizedBanes = 3912996116;
			/// <summary>
			/// 	{var:1027206613}% bonus to Sword damage.
			/// </summary>
			public const uint OverchargedSword = 1326581064;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:219516062}
			/// </summary>
			public const uint ExoticDropRateBoostsGained_2835296134 = 2835296134;
			/// <summary>
			/// 	Round 1: Fallen
			/// <para/>
			/// 	Round 2: Hive
			/// <para/>
			/// 	Final Round: Zydron
			/// <para/>
			/// </summary>
			public const uint ContestantsTheWayoftheHoof_2229025674 = 2229025674;
			/// <summary>
			/// 	{var:1027206613}% bonus to Sniper Rifle damage.
			/// </summary>
			public const uint OverchargedSniperRifle = 1282934989;
			/// <summary>
			/// 	Grenade abilities deal more damage and recharge much faster.
			/// </summary>
			public const uint Grenadier = 2050297303;
			/// <summary>
			/// 	To win, rack up points by defeating your opponents. You gain more points and a boost of energy at each final blow while on a streak of 2 or more.
			/// <para/>
			/// 	Streak: The number of opponents you defeat in a single life. Streaks are broken into 2 tiers called Surging and Primed that award you increasingly more points and energy at each final blow. But beware: once you become Primed, you need to keep quickly defeating your opponents or you'll erupt!
			/// <para/>
			/// 	The Hunt: This activates for your team when you or your teammate is Primed by reaching a streak of 5 or more. At the start of The Hunt, the whole team gets a boost of energy.  It ends when your team has no more players Primed.
			/// <para/>
			/// </summary>
			public const uint IronBannerEruption = 372037179;
			/// <summary>
			/// 	Weapon overcharges from the Seasonal Artifact are active in this activity.
			/// <para/>
			/// 	Kinetic weapons do increased damage when your Super element matches an active surge.
			/// <para/>
			/// </summary>
			public const uint OverchargedWeapons = 4226469317;
			/// <summary>
			/// 	The activity is at a fixed difficulty. Test your might.
			/// </summary>
			public const uint ContestMode_3620565640 = 3620565640;
			/// <summary>
			/// 	Opponent Shields Disabled
			/// </summary>
			public const uint ExpertModifiers_3897480986 = 3897480986;
			/// <summary>
			/// 	Matchmaking selects players based on character class. Each team has one of each class.
			/// </summary>
			public const uint Matchmaking_302322071 = 302322071;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint ZerotoOneHundred = 4073197101;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier, [Disruption] Overload, and [Stagger] Unstoppable Champions.
			/// </summary>
			public const uint ChampionFoes_4038464106 = 4038464106;
			/// <summary>
			/// 	{var:1027206613}% bonus to Shotgun damage.
			/// </summary>
			public const uint OverchargedShotgun = 3132780533;
			/// <summary>
			/// 	Depositing data will award one of the following buffs:
			/// <para/>
			/// 	• Chance to create Threadlings on Strand damage
			/// <para/>
			/// 	• Chance to apply Suppressed on Void damage
			/// <para/>
			/// 	• Chance to gain grenade energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_3507208337 = 3507208337;
			/// <summary>
			/// 	Using class abilities increases your fireteam's damage output. Reduces incoming damage while active.
			/// </summary>
			public const uint ClassWarfare = 2117382464;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_2768123075 = 2768123075;
			/// <summary>
			/// 	Combatant difficulty is set at a fixed level.
			/// </summary>
			public const uint PowerLevelDisabled = 1410302403;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint UnderConstruction = 2936050162;
			/// <summary>
			/// 	Matchmaking selects players based on character class. Each team is formed entirely of one class type.
			/// </summary>
			public const uint Matchmaking_100355054 = 100355054;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint TotheTop = 2626004713;
			/// <summary>
			/// 	Melee abilities deal more damage and recharge much faster.
			/// </summary>
			public const uint Brawler = 1376889134;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint TheGrassIsAlwaysGreener = 3577304467;
			/// <summary>
			/// 	Players do not respawn with Special ammo.
			/// </summary>
			public const uint SpecialAmmoRulesAlpha = 3492492287;
			/// <summary>
			/// 	Savathûn has loaned you a worm. When it hungers, deal damage of the focused type to satiate it. When the worm is full, your ability regeneration is increased. If the worm starves, your ability regeneration is reduced and you take increased damage from the focused damage type.
			/// </summary>
			public const uint ArcHunger = 2081001771;
			/// <summary>
			/// 	+50% Stasis effectiveness.
			/// </summary>
			public const uint StasisBurn = 838742970;
			/// <summary>
			/// 	Guardians deal increased damage with Auto Rifles and Grenade Launchers.
			/// </summary>
			public const uint MA75Multipurpose = 1926594730;
			/// <summary>
			/// 	Round 1: Vex
			/// <para/>
			/// 	Round 2: Cabal
			/// <para/>
			/// 	Final Round: Crota
			/// <para/>
			/// </summary>
			public const uint ContestantsDisciplesoftheNeigh_1456687498 = 1456687498;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:733308108}
			/// </summary>
			public const uint BoostsGained_3901838854 = 3901838854;
			/// <summary>
			/// 	This week, an Elder Revenant awaits you on wave 50.
			/// </summary>
			public const uint WeeklyBoss_4105691942 = 4105691942;
			/// <summary>
			/// 	This playlist contains Control and Clash. When you launch, it will randomly select one of the modes for you to play.
			/// <para/>
			/// 	Control: Capture the zones and defeat your opponents for points. The more zones you control, the more points you get for each opponent defeated. The team with the most points wins!
			/// <para/>
			/// 	Clash: To win, defeat your opponents to score more points than them!
			/// <para/>
			/// </summary>
			public const uint Quickplay6v6Rules = 3192928767;
			/// <summary>
			/// 	{var:2189146210}% bonus to outgoing Stasis damage.
			/// </summary>
			public const uint StasisSurge = 3809788899;
			/// <summary>
			/// 	You will not be able to change your equipment after this activity starts.
			/// </summary>
			public const uint EquipmentLocked = 1486810101;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint StrangersinTime = 3212721586;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint OfAllTrades = 201968501;
			/// <summary>
			/// 	Flying units do increased damage.
			/// </summary>
			public const uint AirSuperiority = 68890788;
			/// <summary>
			/// 	{var:1027206613}% bonus to Pulse Rifle damage.
			/// </summary>
			public const uint OverchargedPulseRifle = 2984170047;
			/// <summary>
			/// 	All Gambit Rank point gains are increased!
			/// </summary>
			public const uint IncreasedGambitRank = 3228023383;
			/// <summary>
			/// 	If all your fireteam members fall with 0 Revive Tokens, everyone is returned to orbit.
			/// </summary>
			public const uint FullExtinguish = 1852184322;
			/// <summary>
			/// 	{var:1027206613}% bonus to Auto Rifle damage.
			/// </summary>
			public const uint OverchargedAutoRifle = 3406250074;
			/// <summary>
			/// 	Extra Champions
			/// <para/>
			/// 	Locked Loadout
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint MasterModifiers_3623371497 = 3623371497;
			/// <summary>
			/// 	When defeated, combatants can drop explosives disguised as familiar items.
			/// </summary>
			public const uint Counterfeit = 1279873648;
			/// <summary>
			/// 	To win, your team must detonate or defuse both charges.
			/// <para/>
			/// 	Attackers: Arm the charge at one of two sites and guard it until it detonates. 
			/// <para/>
			/// 	Defenders (Arming Phase): Prevent the attacking team from arming any charges until the round ends.  
			/// <para/>
			/// 	Defenders (Defusing Phase): Defuse the armed charge.
			/// <para/>
			/// 	Respawns Enabled: Defeated Guardians will respawn after a short time.
			/// <para/>
			/// </summary>
			public const uint CountdownRules_2960104736 = 2960104736;
			/// <summary>
			/// 	Base health regen is reduced. If near another player, health regen is increased.
			/// </summary>
			public const uint Togetherness = 3629079662;
			/// <summary>
			/// 	Power is disabled in this activity. Combatant difficulty increases with each set.
			/// </summary>
			public const uint EscalatingDifficulty_3114697541 = 3114697541;
			/// <summary>
			/// 	Swapping Exotic armor drains all ability energy.
			/// </summary>
			public const uint Notswap = 3792281695;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_1396678533 = 1396678533;
			/// <summary>
			/// 	Fireteam revives are limited in Restricted Zones.
			/// </summary>
			public const uint Restricted = 1909158654;
			/// <summary>
			/// 	When close to a fireteam member, gain increased Recovery and Super regeneration.
			/// </summary>
			public const uint Flare_874265159 = 874265159;
			/// <summary>
			/// 	You will face combatants with [Void] Void shields.
			/// <para/>
			/// 	Activity modifiers may add more shield types.
			/// <para/>
			/// </summary>
			public const uint ShieldedFoes_3538098588 = 3538098588;
			/// <summary>
			/// 	When defeated, Acolytes spawn fire pools that cause damage over time.
			/// </summary>
			public const uint FirePit = 2139004924;
			/// <summary>
			/// 	Keep it secret.  Keep it safe.
			/// </summary>
			public const uint Classified_1897835427 = 1897835427;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier and [Disruption] Overload Champions.
			/// </summary>
			public const uint ChampionFoes_197794292 = 197794292;
			/// <summary>
			/// 	You will face combatants with [Arc] Arc, [Solar] Solar, and [Void] Void shields.
			/// </summary>
			public const uint ShieldedFoes_1651706850 = 1651706850;
			/// <summary>
			/// 	Depositing data will award one of the following buffs:
			/// <para/>
			/// 	• Chance to spawn an Orb of Power on combatant final blows
			/// <para/>
			/// 	• Chance to spawn a Stasis Crystal on combatant final blows
			/// <para/>
			/// 	• Chance to gain class ability energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_1471048891 = 1471048891;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint GazeAmaze = 2890753840;
			/// <summary>
			/// 	Rapidly defeat combatants to unleash waves of paracausal fire around you dealing damage to nearby combatants.
			/// </summary>
			public const uint Flare_1969121547 = 1969121547;
			/// <summary>
			/// 	"Salvation's Edge" raid challenge modes.
			/// </summary>
			public const uint ScenicRouteChallenge = 3621023005;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint StrengthofMemory = 1498263100;
			/// <summary>
			/// 	Combatants have more health and are more difficult to stun.
			/// </summary>
			public const uint Galvanized = 941999846;
			/// <summary>
			/// 	You will face combatants with [Solar] Solar and [Void] Void shields.
			/// <para/>
			/// 	Activity modifiers may add more shield types.
			/// <para/>
			/// </summary>
			public const uint ShieldedFoes_3958417570 = 3958417570;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:1641349362}
			/// </summary>
			public const uint BoostsGained_1874523520 = 1874523520;
			/// <summary>
			/// 	You will face combatants with [Solar] Solar shields.
			/// <para/>
			/// 	Activity modifiers may add more shield types.
			/// <para/>
			/// </summary>
			public const uint ShieldedFoes_1553093202 = 1553093202;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_1922761042 = 1922761042;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint RedRover = 4023623132;
			/// <summary>
			/// 	You will face [Stagger] Unstoppable Champions.
			/// </summary>
			public const uint ChampionFoes_1598783516 = 1598783516;
			/// <summary>
			/// 	Damage taken while you are airborne is greatly increased.
			/// </summary>
			public const uint Grounded = 261995644;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:3027371367}
			/// </summary>
			public const uint BoostsGained_4075903438 = 4075903438;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier and [Stagger] Unstoppable Champions.
			/// </summary>
			public const uint ChampionFoes_1806568190 = 1806568190;
			/// <summary>
			/// 	A Fieldrazer Tormentor awaits you on wave 50.
			/// </summary>
			public const uint BossRotation_1743397814 = 1743397814;
			/// <summary>
			/// 	Your current character has not yet completed this mission on Legendary difficulty.
			/// <para/>
			/// 	Completing all Legendary missions provides additional rewards per class.
			/// <para/>
			/// </summary>
			public const uint AChallengeAwaits_22073894 = 22073894;
			/// <summary>
			/// 	Arc damage increases slightly from all sources.
			/// </summary>
			public const uint ArcSinge = 4162874069;
			/// <summary>
			/// 	A Fallen Warpriest awaits you on wave 50.
			/// </summary>
			public const uint BossRotation_2160690722 = 2160690722;
			/// <summary>
			/// 	{var:4005007457}% increase to incoming Stasis damage.
			/// </summary>
			public const uint StasisThreat = 512042454;
			/// <summary>
			/// 	{var:1027206613}% bonus to Fusion Rifle damage.
			/// </summary>
			public const uint OverchargedFusionRifle = 2626834038;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:3177264920}
			/// </summary>
			public const uint ExoticDropRateBoostsGained_3076996298 = 3076996298;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier Champions.
			/// </summary>
			public const uint ChampionFoes_1615778293 = 1615778293;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint Leftovers = 584466411;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint EnsemblesRefrain = 1572570117;
			/// <summary>
			/// 	Defend and repair the ADU against waves of combatants. There are 10 waves to a set. If the ADU is destroyed, the activity is over.
			/// <para/>
			/// 	Scraps: Activity currency used to purchase and upgrade defenses at the start of every set and after waves 3 and 6.
			/// <para/>
			/// 	Rewards: Received after every set upon defeating the boss wave. Subsequent sets provide better rewards. Expert mode increases rewards.
			/// <para/>
			/// 	Bonus Objectives: These reward resources and armaments.
			/// <para/>
			/// </summary>
			public const uint OnslaughtRules_2543520884 = 2543520884;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier and [Disruption] Overload Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_1990363418 = 1990363418;
			/// <summary>
			/// 	{var:2189146210}% bonus to outgoing Strand damage.
			/// </summary>
			public const uint StrandSurge = 3810297122;
			/// <summary>
			/// 	You will face [Disruption] Overload Champions.
			/// </summary>
			public const uint ChampionFoes_3461252634 = 3461252634;
			/// <summary>
			/// 	If your fireteam falls in a Restricted Zone, everyone is returned to orbit.
			/// </summary>
			public const uint Extinguish_2889256157 = 2889256157;
			/// <summary>
			/// 	Win 2 of the next 3 matches to avoid relegation.
			/// <para/>
			/// 	Match results (W-L): {var:1550688411}-{var:3875673079}
			/// <para/>
			/// </summary>
			public const uint RelegationSeries = 3993408768;
			/// <summary>
			/// 	You will not be able to change your equipped subclass during this activity.
			/// </summary>
			public const uint SubclassLocked = 3037208116;
			/// <summary>
			/// 	When on an Unflawed Card, matchmaking does not utilize skill.
			/// <para/>
			/// 	When on a Flawed Card, matchmaking prefers players with similar weekly performance in Trials of Osiris.
			/// <para/>
			/// </summary>
			public const uint Matchmaking_1870323889 = 1870323889;
			/// <summary>
			/// 	Shanks now have Solar shields
			/// </summary>
			public const uint HotKnife = 112345143;
			/// <summary>
			/// 	All Crucible loot drops are doubled on wins.
			/// </summary>
			public const uint DoubleCrucibleDrops = 3619879173;
			/// <summary>
			/// 	Your health is massively increased but does not regenerate. Defeating challenging combatants will recover health for you and nearby allies.
			/// </summary>
			public const uint Brawn = 2042632950;
			/// <summary>
			/// 	Keep it secret.  Keep it safe.
			/// </summary>
			public const uint Classified_3193240035 = 3193240035;
			/// <summary>
			/// 	To win, your team must become the last Guardian(s) standing by defeating the entire opposing team.
			/// <para/>
			/// 	Tiebreaker: A capture zone appears when a round reaches overtime. Win the round by capturing the zone before the other team.
			/// <para/>
			/// </summary>
			public const uint EliminationRules = 3244996239;
			/// <summary>
			/// 	Spawn Cadence: Heavy ammo will spawn on the third round only.
			/// </summary>
			public const uint HeavyAmmoRulesBravo = 707305737;
			/// <summary>
			/// 	Combatants may vary each time you play this activity. You may face combatants with [Arc] Arc, [Solar] Solar, or [Void] Void shields.
			/// </summary>
			public const uint ShieldedFoes_4068824296 = 4068824296;
			/// <summary>
			/// 	The activity is at a fixed difficulty. Test your might.
			/// </summary>
			public const uint ContestMode_678087389 = 678087389;
			/// <summary>
			/// 	To win, your team must become the last Guardian(s) standing by defeating the opposing team to exhaust their revives.
			/// <para/>
			/// 	Revives: Both teams are allocated a limited pool of revives. If a team runs out of their revives, they can no longer respawn.
			/// <para/>
			/// </summary>
			public const uint SurvivalRules = 636485977;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint SwiftDestruction = 2672226122;
			/// <summary>
			/// 	Potential Data buffs:
			/// <para/>
			/// 	• Chance to create Threadlings on Strand damage
			/// <para/>
			/// 	• Chance to apply Suppressed on Void damage
			/// <para/>
			/// 	• Chance to gain grenade energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_324020031 = 324020031;
			/// <summary>
			/// 	Extinguish
			/// <para/>
			/// 	Limited Revives
			/// <para/>
			/// 	Contest
			/// <para/>
			/// 	Join In Progress Disabled
			/// <para/>
			/// 	Extra Champions
			/// <para/>
			/// 	Locked Loadout
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint GrandmasterModifiers_791047754 = 791047754;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier and [Stagger] Unstoppable Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_438106166 = 438106166;
			/// <summary>
			/// 	Heavily shielded and highly aggressive combatants appear more frequently and in greater numbers.
			/// </summary>
			public const uint CooperativeFocusCampaign = 4047266548;
			/// <summary>
			/// 	[Arc] Arc and [Void] Void Shields
			/// </summary>
			public const uint ShieldedFoes_2650740350 = 2650740350;
			/// <summary>
			/// 	Potential Data buffs:
			/// <para/>
			/// 	• Chance to spawn an Orb of Power on combatant final blows
			/// <para/>
			/// 	• Chance to Jolt on Arc Damage
			/// <para/>
			/// 	• Chance to gain melee energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_3398572761 = 3398572761;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint HandsOff = 4152669757;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_2874799584 = 2874799584;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_4190795159 = 4190795159;
			/// <summary>
			/// 	To win, defeat your opponents to score more points than them!
			/// </summary>
			public const uint ClashRules = 1377045711;
			/// <summary>
			/// 	[Arc] Arc, [Solar] Solar, [Void] Void, and Strand Shields
			/// </summary>
			public const uint ShieldedFoes_2585386105 = 2585386105;
			/// <summary>
			/// 	[Void] Void Shields
			/// </summary>
			public const uint ShieldedFoes_720259466 = 720259466;
			/// <summary>
			/// 	Iron Banner Rank point gains are increased by: {var:204478713}.{var:542268544}x
			/// <para/>
			/// 	Challenges Completed: {var:3685276897} of 4
			/// <para/>
			/// 	Iron Banner Gear Equipped: {var:3094895831} of 5
			/// <para/>
			/// 	Iron Banner Emblem: {var:334376694} of 1
			/// <para/>
			/// </summary>
			public const uint IronBannerRankBoost = 2542545137;
			/// <summary>
			/// 	You will face [Disruption] Overload Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_1262171714 = 1262171714;
			/// <summary>
			/// 	Depositing data will award one of the following buffs:
			/// <para/>
			/// 	• Spawn an ammo crate
			/// <para/>
			/// 	• Chance to spawn Threadlings on combatant final blows
			/// <para/>
			/// 	• Chance to gain grenade energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_521652513 = 521652513;
			/// <summary>
			/// 	This mode contains Barrier Champions, which cannot be stopped without an Anti-Barrier mod.
			/// </summary>
			public const uint ChampionsBarrier = 1974619026;
			/// <summary>
			/// 	Enemies have more health and are not staggered by damage.
			/// </summary>
			public const uint Iron = 1463769380;
			/// <summary>
			/// 	Moving quickly slowly regenerates your health. Standing still slowly deals damage.
			/// </summary>
			public const uint Haste = 858319113;
			/// <summary>
			/// 	If your fireteam falls in a Restricted Zone, everyone is returned to orbit.
			/// </summary>
			public const uint Extinguish_4265630562 = 4265630562;
			/// <summary>
			/// 	Your fireteam cannot be joined while in an activity. Return to orbit to add fireteam members.
			/// </summary>
			public const uint JoinInProgressDisabled = 517221851;
			/// <summary>
			/// 	Locked Loadout
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint ExpertModifiers_1174869237 = 1174869237;
			/// <summary>
			/// 	"Salvation's Edge" raid challenge modes.
			/// </summary>
			public const uint CoordinatedEffortsChallenge = 1315424373;
			/// <summary>
			/// 	To win, your team must detonate or defuse charges. Defeating the entire opposing team and being the last Guardian(s) standing will also grant victory.
			/// <para/>
			/// 	Attackers: Arm the charge at one of two sites, and guard it until it detonates. Eliminating the defending team will result in winning a round.
			/// <para/>
			/// 	Defenders (Arming Phase): Prevent the attacking team from arming any charges until the round ends. Eliminating the attacking team will result in winning a round. 
			/// <para/>
			/// 	Defenders (Defusing Phase): Defuse the armed charge. Eliminating the attacking team will not end the round.
			/// <para/>
			/// </summary>
			public const uint CountdownRules_1895965859 = 1895965859;
			/// <summary>
			/// 	Fireteam power boosting is disabled for this activity.
			/// </summary>
			public const uint FireteamPowerBoostingDisabled = 1806775150;
			/// <summary>
			/// 	All Encounter Challenges Enabled and Required
			/// </summary>
			public const uint ChallengeMode_4066301271 = 4066301271;
			/// <summary>
			/// 	All challenges are active.
			/// </summary>
			public const uint RaidChallenges_3119075764 = 3119075764;
			/// <summary>
			/// 	Guardians deal increased damage with Sidearms.
			/// </summary>
			public const uint KeepItLoaded = 1931480264;
			/// <summary>
			/// 	Defend and repair the ADU against waves of combatants. There are 10 waves to a set.
			/// <para/>
			/// 	ADU: If the ADU is destroyed, the activity ends with the set.
			/// <para/>
			/// 	Scraps: In-activity currency earned by scoring. Used to purchase and upgrade defenses at the start of every set and after the third and sixth waves.
			/// <para/>
			/// 	Rewards: Granted on defeating a set's boss wave. Subsequent sets grant better rewards. Expert difficulty increases rewards.
			/// <para/>
			/// 	Bonus Objectives: Reward resources, vendor reputation, and armaments.
			/// <para/>
			/// </summary>
			public const uint OnslaughtRules_1096446210 = 1096446210;
			/// <summary>
			/// 	Keep it secret.  Keep it safe.
			/// </summary>
			public const uint Classified_848207019 = 848207019;
			/// <summary>
			/// 	Enemy shields are highly resistant to all unmatched elemental damage.
			/// </summary>
			public const uint MatchGame = 2751349583;
			/// <summary>
			/// 	Activates on wave 31.
			/// <para/>
			/// 	If your fireteam falls, you will be returned to orbit.
			/// <para/>
			/// </summary>
			public const uint DelayedExtinguish_2081148033 = 2081148033;
			/// <summary>
			/// 	{var:2189146210}% bonus to outgoing Solar damage.
			/// </summary>
			public const uint SolarSurge = 426976067;
			/// <summary>
			/// 	[Arc] Arc and [Solar] Solar Shields
			/// </summary>
			public const uint ShieldedFoes_3119632620 = 3119632620;
			/// <summary>
			/// 	This activity's difficulty has been tuned to approximate the experience of its original release.
			/// <para/>
			/// 	Combatants deal significantly more damage and have moderately increased health.
			/// <para/>
			/// </summary>
			public const uint LegacyTheWhisper = 1602614337;
			/// <summary>
			/// 	Greater fireteam size scales combatant difficulty and adds limitations on revives.
			/// </summary>
			public const uint Multiplicity = 3398421092;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:2223847550}
			/// </summary>
			public const uint BoostsGained_3282103678 = 3282103678;
			/// <summary>
			/// 	Melee units have increased health.
			/// </summary>
			public const uint MeleeHealthIncrease = 2533375223;
			/// <summary>
			/// 	All Nightfall loot drops are doubled.
			/// </summary>
			public const uint DoubleNightfallDrops = 1171597537;
			/// <summary>
			/// 	Win 2 of the next 3 matches to be promoted to the next Rank.
			/// <para/>
			/// 	Match results (W-L): {var:3036110002}-{var:2130373872}
			/// <para/>
			/// </summary>
			public const uint PromotionSeries = 2975928611;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint BaseInformation = 3425521520;
			/// <summary>
			/// 	Extinguish
			/// <para/>
			/// 	Extra Champions
			/// <para/>
			/// 	Locked Loadout
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint MasterModifiers_85104725 = 85104725;
			/// <summary>
			/// 	Radar is disabled.
			/// </summary>
			public const uint Chaff = 3101164718;
			/// <summary>
			/// 	{var:1027206613}% bonus to Machine Gun damage.
			/// </summary>
			public const uint OverchargedMachineGun = 795009574;
			/// <summary>
			/// 	Venture into the VexNet, disrupt the Vex from accessing and locking away a treasure trove of Golden Age data, and investigate strange clues left in the network.
			/// <para/>
			/// 	Enigma Protocol uses a low starting timer, requiring Guardians to move quickly and find ways to increase their time in the VexNet.
			/// <para/>
			/// 	To increase your timer:
			/// <para/>
			/// 	-Destroy Network Security Nodes
			/// <para/>
			/// 	-Defeat Archival Timekeepers
			/// <para/>
			/// 	-Unlock "Codebase Splice" Vendor Upgrades
			/// <para/>
			/// </summary>
			public const uint EnigmaProtocolRules = 1060852100;
			/// <summary>
			/// 	Combatant power increases after each set.
			/// </summary>
			public const uint EscalatingDifficulty_3320264005 = 3320264005;
			/// <summary>
			/// 	All Crucible Rank point gains are increased!
			/// </summary>
			public const uint IncreasedCrucibleRank = 3874605433;
			/// <summary>
			/// 	You will face [Disruption] Overload and [Stagger] Unstoppable Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_3307318061 = 3307318061;
			/// <summary>
			/// 	The activity is set at a fixed difficulty. Test your might.
			/// </summary>
			public const uint ContestMode_1009404927 = 1009404927;
			/// <summary>
			/// 	All ammunition drops are significantly reduced.
			/// </summary>
			public const uint Famine = 965929096;
			/// <summary>
			/// 	Score points by defeating your opponents with a relic.
			/// <para/>
			/// 	Relics: Mysterious weapons that hold immense power. Defeat your opponents with a relic to score points for your team.
			/// <para/>
			/// 	Relic Depot: Each depot can equip you with one of the following relics: Aegis Shield, Synaptic Spear, Solar Scythe.
			/// <para/>
			/// 	Relic Energy: Players without an equipped relic must earn energy to gain access to the Relic Depot. Relic energy can be built up by your own actions, your team's actions, and even the opponent's actions throughout the match.
			/// <para/>
			/// </summary>
			public const uint RelicRules = 4258252763;
			/// <summary>
			/// 	Extra Shields
			/// </summary>
			public const uint ExpertModifiers_2567927655 = 2567927655;
			/// <summary>
			/// 	All collected Elemental Orbs contribute toward the elemental buff counter. Subclass choice determines which elemental buff is applied.
			/// <para/>
			/// 	Prismatic orbs from Taken combatants grant a random empowerment buff.
			/// <para/>
			/// </summary>
			public const uint PRISMDay = 2492586271;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint SummoningRitual = 2834033523;
			/// <summary>
			/// 	A fireteam member needs to be equipped with a Light subclass. The other fireteam member needs to be equipped with a Darkness subclass. The Prismatic subclass fulfills either requirement.
			/// </summary>
			public const uint SubclassRequirements = 3924377323;
			/// <summary>
			/// 	Solar final blows grant melee and grenade energy. Stasis subclasses receive a lot; other subclasses receive a little.
			/// </summary>
			public const uint SolarThermalSwap = 1575279060;
			/// <summary>
			/// 	Health and shields are reduced by half, but recovery is twice as fast.
			/// </summary>
			public const uint Glass = 317879233;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint LoopingCatalyst = 2098788044;
			/// <summary>
			/// 	All challenges are active.
			/// </summary>
			public const uint RaidChallenges_824516719 = 824516719;
			/// <summary>
			/// 	Fireteam communication and coordination is vital for success.
			/// </summary>
			public const uint Synergy = 1149526926;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:2864503325}
			/// </summary>
			public const uint BoostsGained_4232900926 = 4232900926;
			/// <summary>
			/// 	This activity is timed. If your fireteam fails to complete the activity in the allotted time, everyone is returned to orbit.
			/// </summary>
			public const uint OntheClock = 2290945089;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:1968703538}
			/// </summary>
			public const uint BoostsGained_1834872958 = 1834872958;
			/// <summary>
			/// 	Ammo crates spawn much more frequently, and players drop more ammo when defeated.
			/// </summary>
			public const uint HavocRules = 474395167;
			/// <summary>
			/// 	To win, score more points than your opponent by collecting the Spark and igniting your opponent's Rift.
			/// <para/>
			/// 	Spark: Spawns in the center of the map at the beginning of the match and after a team scores. Drops at the Spark Runner's location upon death. 
			/// <para/>
			/// 	Rift: Each team has a Rift on their side of the map. Carry the Spark into your opponent's Rift to score. Defend your Rift from opposing Spark Runners.
			/// <para/>
			/// </summary>
			public const uint RiftRules = 4211958639;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint KeepOut = 1741733430;
			/// <summary>
			/// 	To win, defeat your opponents to get the most points! This is a free-for-all mode where no one is an ally.
			/// </summary>
			public const uint RumbleRules = 1359116199;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint WhichWitch = 217815700;
			/// <summary>
			/// 	Savathûn has loaned you a worm. When it hungers, deal damage of the focused type to satiate it. When the worm is full, your ability regeneration is increased. If the worm starves, your ability regeneration is reduced and you take increased damage from the focused damage type.
			/// </summary>
			public const uint VoidHunger = 976277003;
			/// <summary>
			/// 	All abilities are disabled! Weapons are your only defense.
			/// <para/>
			/// 	Collect Special ammo and Heavy ammo from crates around the map.
			/// <para/>
			/// </summary>
			public const uint HardwareFocus = 433047189;
			/// <summary>
			/// 	Enhanced radar. Take increased damage from melee.
			/// </summary>
			public const uint Empath_166293531 = 166293531;
			/// <summary>
			/// 	Heavily shielded and highly aggressive combatants appear in great numbers.
			/// </summary>
			public const uint Legendary = 36103086;
			/// <summary>
			/// 	This activity's difficulty has been tuned to approximate the experience of its original release.
			/// <para/>
			/// 	Combatants deal significantly more damage and have moderately increased health.
			/// <para/>
			/// </summary>
			public const uint LegacyZeroHour = 2432579025;
			/// <summary>
			/// 	To win, defeat your opponents to score more points than them! Your abilities recharge much faster in this mode, and final blows made with Power weapons or Supers grant additional points.
			/// </summary>
			public const uint MayhemRules = 1792878913;
			/// <summary>
			/// 	You will face combatants with [Arc] Arc and [Void] Void shields.
			/// <para/>
			/// 	Activity modifiers may add more shield types.
			/// <para/>
			/// </summary>
			public const uint ShieldedFoes_2833087500 = 2833087500;
			/// <summary>
			/// 	{var:1027206613}% bonus to Trace Rifle damage.
			/// </summary>
			public const uint OverchargedTraceRifle = 2178457119;
			/// <summary>
			/// 	Power Level: All combatants in this activity will be at least {var:3112783658} points over your current Power level.
			/// </summary>
			public const uint ExoticModifiers = 3674616727;
			/// <summary>
			/// 	Limited fireteam revives. Gain additional revives by defeating Champions.
			/// </summary>
			public const uint LimitedRevives_203094476 = 203094476;
			/// <summary>
			/// 	Round 1: Hive
			/// <para/>
			/// 	Round 2: Fallen
			/// <para/>
			/// 	Final Round: Valus Ta'aurc
			/// <para/>
			/// </summary>
			public const uint ContestantsTheWayoftheHoof_377122366 = 377122366;
			/// <summary>
			/// 	You will face combatants with [Arc] Arc, [Solar] Solar, [Void] Void, and Strand shields.
			/// </summary>
			public const uint ShieldedFoes_1111960127 = 1111960127;
			/// <summary>
			/// 	This activity is currently Focused. Your score will contribute to your class's standing in the fight for glory in the Focus Activity Event.
			/// </summary>
			public const uint FocusedActivity = 344764180;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint CopiesofCopies = 3361897360;
			/// <summary>
			/// 	All challenges are active.
			/// </summary>
			public const uint RaidChallenges_3303073618 = 3303073618;
			/// <summary>
			/// 	You will face [Shield-Piercing] Barrier, [Disruption] Overload, and [Stagger] Unstoppable Champions.
			/// <para/>
			/// 	Visit the Character or Mod Customization screen to view your active anti-Champion perks.
			/// <para/>
			/// </summary>
			public const uint ChampionFoes_2006149364 = 2006149364;
			/// <summary>
			/// 	When close to a fireteam member, gain increased Recovery and Super regeneration.
			/// </summary>
			public const uint Firebound = 998963135;
			/// <summary>
			/// 	This playlist features three new Crucible maps: Dissonance, Eventide Labs, and Cirrus Plaza.
			/// <para/>
			/// 	It features four randomized modes: Collision, Clash, Survival, and Elimination.
			/// <para/>
			/// </summary>
			public const uint NewTerritory = 915150015;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:3104627285}
			/// </summary>
			public const uint BoostsGained_3746163602 = 3746163602;
			/// <summary>
			/// 	Your current character has not yet completed this mission on Legendary difficulty.
			/// <para/>
			/// 	Completing all Legendary missions provides additional rewards per class.
			/// <para/>
			/// </summary>
			public const uint AChallengeAwaits_384012338 = 384012338;
			/// <summary>
			/// 	When the timer reaches zero, the fireteam will be returned to orbit.
			/// </summary>
			public const uint MissionIsTimed = 4115568531;
			/// <summary>
			/// 	Solar damage increases slightly from all sources.
			/// </summary>
			public const uint SolarSinge = 1879607770;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint ALinktotheChain = 787678752;
			/// <summary>
			/// 	Extra Shields
			/// </summary>
			public const uint AdvancedModifiers = 3240131679;
			/// <summary>
			/// 	Complete objectives and defeat high-value targets to generate data Motes.
			/// <para/>
			/// 	Upon death, some collected data will drop to the ground to be reacquired.
			/// <para/>
			/// 	Pick up data and deposit it to earn power-ups during encounters.
			/// <para/>
			/// 	Successfully completing encounters in Expert difficulty will provide additional rewards at the end of the activity.
			/// <para/>
			/// </summary>
			public const uint ArenaBreachExecutableModeRules = 2431130026;
			/// <summary>
			/// 	To win, defeat your opponents to score more points than them! Every player is equipped with a Scorch Cannon.
			/// <para/>
			/// 	Scorch Cannon: A heavy-impact weapon borrowed from the Eliksni. Shots can detonate on impact or can be charged by holding and releasing the actuator for a larger explosion.
			/// <para/>
			/// </summary>
			public const uint TeamScorchedRules = 3486817119;
			/// <summary>
			/// 	When defeated, Fallen Vandals spawn a web mine at their feet.
			/// </summary>
			public const uint ArachNO = 3021161702;
			/// <summary>
			/// 	Depositing data will award one of the following buffs:
			/// <para/>
			/// 	• Chance to create Stasis Crystals on Stasis damage
			/// <para/>
			/// 	• Chance to apply Scorched on Solar damage
			/// <para/>
			/// 	• Chance to gain class ability energy on combatant final blows
			/// <para/>
			/// </summary>
			public const uint DataBuffs_2263762817 = 2263762817;
			/// <summary>
			/// 	{var:4005007457}% increase to incoming Void damage.
			/// </summary>
			public const uint VoidThreat = 3652821947;
			/// <summary>
			/// 	When you're holding a Sword, damage from nearby combatants is reduced.
			/// </summary>
			public const uint DurandalsEdge = 3805176141;
			/// <summary>
			/// 	You may face combatants with [Arc] Arc, [Solar] Solar, or [Void] Void shields.
			/// </summary>
			public const uint ShieldedFoes_93790318 = 93790318;
			/// <summary>
			/// 	You will face combatants with [Arc] Arc shields.
			/// <para/>
			/// 	Activity modifiers may add more shield types.
			/// <para/>
			/// </summary>
			public const uint ShieldedFoes_3139381566 = 3139381566;
			/// <summary>
			/// 	All Encounter Challenges Enabled and Required
			/// </summary>
			public const uint ChallengeMode_1271621951 = 1271621951;
			/// <summary>
			/// 	Initial Exotic drop rate boosts: {var:2935433793}
			/// </summary>
			public const uint BoostsGained_1172122584 = 1172122584;
			public const uint H1783825372 = 1783825372;
			/// <summary>
			/// 	Landing multiple precision hits with a Primary ammo weapon empowers Special and Heavy ammo weapons. Reduces incoming damage while active.
			/// </summary>
			public const uint ShotCaller = 1499411920;
			/// <summary>
			/// 	A challenge awaits…
			/// </summary>
			public const uint AllHandsChallenge = 1616571513;
			/// <summary>
			/// 	Taken Vandals summon their shields significantly more often.
			/// </summary>
			public const uint Denial = 1535174577;
			/// <summary>
			/// 	This mode contains Overload Champions, which cannot be stopped without an Overload mod.
			/// </summary>
			public const uint ChampionsOverload = 1201462052;
			/// <summary>
			/// 	Abilities recharge at a reduced rate.
			/// </summary>
			public const uint Trickle = 3388957297;
			/// <summary>
			/// 	The activity is at a fixed difficulty.
			/// <para/>
			/// 	Your effective Power is capped at {var:1311088063}.
			/// <para/>
			/// </summary>
			public const uint Mettle_1593143649 = 1593143649;
			/// <summary>
			/// 	Stoke the bonfire!
			/// <para/>
			/// 	Ignition Carriers: Glowing combatants that drop Igniters when dealt final blows.
			/// <para/>
			/// 	Igniters: Objects that can be picked up and thrown into the bonfire to stoke the flames.
			/// <para/>
			/// 	Bonfire: Stoke the bonfire with Igniters to make the flames grow.
			/// <para/>
			/// 	Taken Interference: Taken will disrupt the bonfire throughout the activity. Defeat them to reignite the bonfire!
			/// <para/>
			/// </summary>
			public const uint BonfireBashRules = 3186900190;
			/// <summary>
			/// 	The activity is at a fixed difficulty. Test your might.
			/// </summary>
			public const uint ContestMode_2349614593 = 2349614593;
			/// <summary>
			/// 	{var:1027206613}% bonus to Rocket Launcher damage.
			/// </summary>
			public const uint OverchargedRocketLauncher = 95459596;
			/// <summary>
			/// 	Capture and control active zones to score points. There are multiple zone locations, but only one active at a time. 
			/// <para/>
			/// 	The team with the most points at the end of the match wins! Defeating your opponents does not grant points. 
			/// <para/>
			/// 	Zones: Score +1 point when your team captures an active zone. Score +1 point for each time interval your team controls that zone. Captured zones will deactivate after a set amount of time and a new zone will activate elsewhere on the map.
			/// <para/>
			/// </summary>
			public const uint CollisionRules = 1514862847;
			/// <summary>
			/// 	Supers recharge more quickly.
			/// </summary>
			public const uint AvatarasPsyche = 2471054449;
			/// <summary>
			/// 	Void damage increases slightly from all sources.
			/// </summary>
			public const uint VoidSinge = 2620827383;
			/// <summary>
			/// 	Potential Data buffs:
			/// <para/>
			/// 	• Chance to create Stasis Crystals on Stasis damage
			/// <para/>
			/// 	• Chance to apply Scorched on Solar damage
			/// <para/>
			/// 	• Defeated combatants have a chance to refund a portion of class ability energy
			/// <para/>
			/// </summary>
			public const uint DataBuffs_1332265391 = 1332265391;
			/// <summary>
			/// 	Contest
			/// <para/>
			/// 	Join In Progress Disabled
			/// <para/>
			/// 	Extra Champions
			/// <para/>
			/// 	Extra Shields
			/// <para/>
			/// </summary>
			public const uint GrandmasterModifiers_445825536 = 445825536;
		}
	}
}
