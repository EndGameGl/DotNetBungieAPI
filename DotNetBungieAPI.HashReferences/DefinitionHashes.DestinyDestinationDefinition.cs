namespace DotNetBungieAPI.HashReferences
{
	public static partial class DefinitionHashes
	{
		public static class Destinations
		{
			/// <summary>
			/// 	A patchwork wasteland on the edge of the Reef, in the Asteroid Belt.
			/// </summary>
			public const uint TheTangledShore_3821439926 = 3821439926;
			public const uint H1072559238 = 1072559238;
			public const uint H71557692 = 71557692;
			public const uint H2341115919 = 2341115919;
			/// <summary>
			/// 	Back from the abyss, Mars has been altered by an unknown force.
			/// </summary>
			public const uint Mars_1615213322 = 1615213322;
			/// <summary>
			/// 	The transfigured remains of Calus's Leviathan.
			/// </summary>
			public const uint DerelictLeviathan_1446501763 = 1446501763;
			/// <summary>
			/// 	Earth
			/// </summary>
			public const uint EuropeanDeadZone_697502628 = 697502628;
			/// <summary>
			/// 	The transfigured remains of Calus's Leviathan.
			/// </summary>
			public const uint DerelictLeviathan_4081978654 = 4081978654;
			/// <summary>
			/// 	Hunt hostiles in active warzones. Reckon with greater powers. Be notorious.
			/// </summary>
			public const uint TheDerelict_248695599 = 248695599;
			/// <summary>
			/// 	A ceremonial combat theater designed for training and meditation.
			/// </summary>
			public const uint EuropeanAerialZone_3158922420 = 3158922420;
			/// <summary>
			/// 	A long-hidden stronghold sacred to the Awoken of the Reef.
			/// </summary>
			public const uint TheDreamingCity_1416096592 = 1416096592;
			/// <summary>
			/// 	The inner workings of Savathûn's mind, brought to life within a mysterious throne world of her own making.
			/// </summary>
			public const uint SavathûnsThroneWorld = 2244580325;
			/// <summary>
			/// 	New Pacific Archology, Titan
			/// </summary>
			public const uint DeepSix = 1590276;
			public const uint H4256664283 = 4256664283;
			public const uint H1955695930 = 1955695930;
			/// <summary>
			/// 	Home of the Guardians. Gather beneath the Traveler to regroup, rearm, and form new alliances.
			/// </summary>
			public const uint TheLastCityEarth = 2612139675;
			/// <summary>
			/// 	Hellas Basin, Mars
			/// </summary>
			public const uint NewArcadia = 3004560036;
			public const uint Mercury_1259908504 = 1259908504;
			/// <summary>
			/// 	Last City, Earth
			/// </summary>
			public const uint TheLastCity_1272484305 = 1272484305;
			public const uint H3669933163 = 3669933163;
			public const uint DreadnaughtRingsofSaturn = 4173149923;
			/// <summary>
			/// 	Venture into the mysterious Haul that the Drifter tows behind his ship to reckon with greater powers.
			/// </summary>
			public const uint TheReckoning_3005692706 = 3005692706;
			public const uint H328699611 = 328699611;
			public const uint H3670157546 = 3670157546;
			public const uint TheMoon_290444260 = 290444260;
			public const uint H2616293043 = 2616293043;
			public const uint H1756262061 = 1756262061;
			public const uint H1749218505 = 1749218505;
			public const uint H2239137327 = 2239137327;
			public const uint Mercury_1993421442 = 1993421442;
			public const uint H2961497387 = 2961497387;
			/// <summary>
			/// 	Mars
			/// </summary>
			public const uint HellasBasin_308080871 = 308080871;
			/// <summary>
			/// 	Hone your skills and win glory in battle against other Guardians.
			/// </summary>
			public const uint TheCrucible_2777041980 = 2777041980;
			/// <summary>
			/// 	An interstellar raiding vessel crewed by privateers.
			/// </summary>
			public const uint PirateKetch = 1138538228;
			/// <summary>
			/// 	Last City, Earth
			/// </summary>
			public const uint TheLastCity_2158693219 = 2158693219;
			public const uint H3256202400 = 3256202400;
			/// <summary>
			/// 	Join a fireteam of three and face the most feared minions of the Darkness.
			/// </summary>
			public const uint StrikePlaylist = 2512542997;
			/// <summary>
			/// 	Saturn's largest moon, with a lost Golden Age utopia beneath its waves.
			/// </summary>
			public const uint NewPacificArcology_1455639803 = 1455639803;
			public const uint H3507009564 = 3507009564;
			public const uint H3989677910 = 3989677910;
			public const uint H1017151023 = 1017151023;
			/// <summary>
			/// 	Io, Moon of Jupiter
			/// </summary>
			public const uint EchoMesa_763066127 = 763066127;
			public const uint H3536553558 = 3536553558;
			/// <summary>
			/// 	The Emperor's Flagship
			/// </summary>
			public const uint NessusOrbit_3862550981 = 3862550981;
			public const uint H3031909699 = 3031909699;
			/// <summary>
			/// 	An infamous orbiting fortress where the Reef keeps its most dangerous enemies.
			/// </summary>
			public const uint PrisonofElders_2038451695 = 2038451695;
			public const uint H4102725222 = 4102725222;
			/// <summary>
			/// 	Tangled Shore, The Reef
			/// </summary>
			public const uint KellsGrave = 1621667531;
			/// <summary>
			/// 	A derelict spaceflight complex in the ruins of Old Russia.
			/// </summary>
			public const uint Cosmodrome = 3990611421;
			public const uint H1585231233 = 1585231233;
			public const uint H2039972170 = 2039972170;
			/// <summary>
			/// 	Dominus Ghaul's Flagship
			/// </summary>
			public const uint TheAlmighty = 4161571679;
			public const uint H1890123761 = 1890123761;
			/// <summary>
			/// 	Paracausal phenomenon created by the Nine.
			/// </summary>
			public const uint IXRealms_1553550479 = 1553550479;
			/// <summary>
			/// 	Choose your role in this intense single-round evolution of Gambit. Send Blockers to drain Motes from the enemy's bank. Create a Well of Light by defeating envoys. Stand in the Well to burn down the Primeval and win.
			/// </summary>
			public const uint GambitPrime = 897844116;
			/// <summary>
			/// 	Venus was once the site of great discovery—a paradise. Now, it is a monument to all that we have lost. The old stories say we built an academy dedicated to learning and research on the shore of a wondrous sea, and that the waters themselves boiled and rose up to shatter the coast, washing away all that we had built.
			/// </summary>
			public const uint IshtarSinkVenus = 3871070152;
			public const uint H2454401299 = 2454401299;
			public const uint H329262012 = 329262012;
			public const uint H2201105581 = 2201105581;
			/// <summary>
			/// 	Mars
			/// </summary>
			public const uint HellasBasin_2527054255 = 2527054255;
			public const uint H4016153074 = 4016153074;
			/// <summary>
			/// 	A ceremonial combat theater designed for training and meditation.
			/// </summary>
			public const uint EuropeanAerialZone_1471216408 = 1471216408;
			/// <summary>
			/// 	A refugee camp set up during the Red War for Guardians and non Guardians alike, on the outskirts of the EDZ.
			/// </summary>
			public const uint TheFarm_4188263703 = 4188263703;
			public const uint H1124017683 = 1124017683;
			public const uint H2709666235 = 2709666235;
			/// <summary>
			/// 	The Nine have sent an Emissary to test Guardians.
			/// </summary>
			public const uint UnknownSpace_4276116472 = 4276116472;
			/// <summary>
			/// 	Paracausal phenomenon created by the Nine.
			/// </summary>
			public const uint IXRealms_366956745 = 366956745;
			/// <summary>
			/// 	A strange pocket dimension where multiple realities from across the paraverse converge and overlap.
			/// </summary>
			public const uint Eternity = 1449984089;
			/// <summary>
			/// 	Saturn's largest moon, with a lost Golden Age utopia beneath its waves.
			/// </summary>
			public const uint NewPacificArcology_2388758973 = 2388758973;
			/// <summary>
			/// 	Hone your skills and win glory in battle against other Guardians.
			/// </summary>
			public const uint TheCrucible_4088006058 = 4088006058;
			public const uint HELM = 3929841832;
			public const uint H3068492605 = 3068492605;
			/// <summary>
			/// 	Join a fireteam of three and face the forces of Empress Caiatl.
			/// </summary>
			public const uint BattlegroundsPlaylist = 2004354453;
			public const uint TheMoon_677774031 = 677774031;
			public const uint H482721436 = 482721436;
			/// <summary>
			/// 	A remnant of the Black Fleet situated on the frozen tundra of Europa.
			/// </summary>
			public const uint TheEuropanPyramid = 3354423374;
			/// <summary>
			/// 	The ruins of an old Russian spaceport.
			/// </summary>
			public const uint TheCosmodrome = 2073151843;
			public const uint H1976362504 = 1976362504;
			/// <summary>
			/// 	An infamous orbiting fortress where the Reef keeps its most dangerous enemies.
			/// </summary>
			public const uint PrisonofElders_251989860 = 251989860;
			/// <summary>
			/// 	The Emperor's Flagship
			/// </summary>
			public const uint NessusOrbit_3093898507 = 3093898507;
			public const uint H1119773799 = 1119773799;
			/// <summary>
			/// 	Nessus, Unstable Centaur
			/// </summary>
			public const uint ArcadianValley_126924919 = 126924919;
			/// <summary>
			/// 	European Dead Zone, Earth
			/// </summary>
			public const uint EmeraldCoast = 1229692670;
			/// <summary>
			/// 	Venture into the mysterious Haul that the Drifter tows behind his ship to reckon with greater powers.
			/// </summary>
			public const uint TheReckoning_357679773 = 357679773;
			/// <summary>
			/// 	Pleasure and delight await you.
			/// </summary>
			public const uint NessusOrbit_2155983995 = 2155983995;
			/// <summary>
			/// 	Hunt hostiles in active warzones. Reckon with greater powers. Be notorious.
			/// </summary>
			public const uint TheDerelict_308974318 = 308974318;
			/// <summary>
			/// 	A refugee camp set up during the Red War for Guardians and non Guardians alike, on the outskirts of the EDZ.
			/// </summary>
			public const uint TheFarm_2948602617 = 2948602617;
			public const uint H1775713152 = 1775713152;
			/// <summary>
			/// 	The Dreaming City, The Vestian Web
			/// </summary>
			public const uint CathedralofScars = 620798773;
			/// <summary>
			/// 	Back from the abyss, Mars has been altered by an unknown force.
			/// </summary>
			public const uint Mars_2023225122 = 2023225122;
			/// <summary>
			/// 	Last City, Earth
			/// </summary>
			public const uint TheLastCity_1737926756 = 1737926756;
			/// <summary>
			/// 	A long-hidden stronghold sacred to the Awoken of the Reef.
			/// </summary>
			public const uint TheDreamingCity_2779202173 = 2779202173;
			public const uint H1782858935 = 1782858935;
			public const uint H3075136232 = 3075136232;
			/// <summary>
			/// 	Nessus, Unstable Centaur
			/// </summary>
			public const uint ArcadianValley_3607432451 = 3607432451;
			/// <summary>
			/// 	The Nine have sent an Emissary to test Guardians.
			/// </summary>
			public const uint UnknownSpace_1402606356 = 1402606356;
			/// <summary>
			/// 	Io, Moon of Jupiter
			/// </summary>
			public const uint EchoMesa_2218917881 = 2218917881;
			/// <summary>
			/// 	Join a fireteam of three and face the most feared minions of the Darkness.
			/// </summary>
			public const uint VanguardOpsPlaylist = 3129078390;
			/// <summary>
			/// 	Back from the abyss, Mars has been altered by an unknown force.
			/// </summary>
			public const uint Mars_1925528349 = 1925528349;
			/// <summary>
			/// 	A patchwork wasteland on the edge of the Reef, in the Asteroid Belt.
			/// </summary>
			public const uint TheTangledShore_359854275 = 359854275;
			/// <summary>
			/// 	Group of main-belt asteroids, frequented by unsavory privateers.
			/// </summary>
			public const uint ThemisCluster = 524525790;
			/// <summary>
			/// 	Last City, Earth
			/// </summary>
			public const uint TheLastCity_333456177 = 333456177;
			/// <summary>
			/// 	Earth
			/// </summary>
			public const uint EuropeanDeadZone_1199524104 = 1199524104;
			public const uint H3592473643 = 3592473643;
			public const uint H3611842038 = 3611842038;
			public const uint H159602672 = 159602672;
			public const uint H684141231 = 684141231;
			/// <summary>
			/// 	A desolate and frozen moon, testament to humanity's ambition and hubris.
			/// </summary>
			public const uint Europa = 1729879943;
			public const uint H4067383504 = 4067383504;
			/// <summary>
			/// 	Join a fireteam to overcome challenges and crush your foes.
			/// </summary>
			public const uint SeasonalActivity = 969538215;
			/// <summary>
			/// 	Arcadian Valley, Nessus
			/// </summary>
			public const uint LegionsFolly = 123491860;
			/// <summary>
			/// 	Long-forgotten Warsat orbital launch facility.
			/// </summary>
			public const uint SeraphFacility = 3832342365;
		}
	}
}
