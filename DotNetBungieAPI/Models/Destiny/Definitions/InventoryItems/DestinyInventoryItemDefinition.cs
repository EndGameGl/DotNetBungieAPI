    ...
    IDeepEquatable<DestinyInventoryItemDefinition>
{
    ...
    public void MapValues()
    {
        ...
        foreach (var itemCategory in ItemCategories) itemCategory.TryMapValue();
        ...
        if (Quality != null)
        {
            ...
            foreach (var version in Quality.Versions) version.PowerCap.TryMapValue();
        }

        if (Objectives != null)
        {
            foreach (var activity in Objectives.DisplayActivities) activity.TryMapValue();

            foreach (var objective in Objectives.Objectives) objective.TryMapValue();

            Objectives.QuestlineItem.TryMapValue();
            foreach (var property in Objectives.PerObjectiveDisplayProperties) property.Activity.TryMapValue();
        }
        ...
        if (Action != null)
        {
            foreach (var reward in Action.ProgressionRewards) reward.ProgressionMapping.TryMapValue();

            foreach (var item in Action.RequiredItems) item.Item.TryMapValue();
        ...
        if (Sockets is not null)
        {
            ...
            foreach (var socketCategory in Sockets.SocketCategories) socketCategory.SocketCategory.TryMapValue();
        ...
        foreach (var stat in InvestmentStats) stat.StatType.TryMapValue();

        foreach (var perk in Perks) perk.Perk.TryMapValue();
