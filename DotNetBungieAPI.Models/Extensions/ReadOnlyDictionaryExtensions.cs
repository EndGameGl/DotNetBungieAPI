...
public static class ReadOnlyDictionaryExtensions
{
    public static bool DeepEqualsReadOnlyDictionaryWithDefinitionKeyAndSimpleValue<T, P>(
        this ReadOnlyDictionary<T, P> compared,
        ReadOnlyDictionary<T, P> comparedWith
    )
        where T : IDeepEquatable<T>
    {
        ...
        for (var i = 0; i < compared.Count; i++)
            if (
                !compared.ElementAt(i).Key.DeepEquals(comparedWith.ElementAt(i).Key)
        ...
        where P : IDeepEquatable<P>
    {
        ...
        for (var i = 0; i < compared.Count; i++)
            if (
                !compared.ElementAt(i).Value.DeepEquals(comparedWith.ElementAt(i).Value)
