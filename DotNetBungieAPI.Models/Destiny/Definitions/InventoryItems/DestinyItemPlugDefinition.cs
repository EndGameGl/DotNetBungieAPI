...
/// <summary>
///     If an item is a Plug, its DestinyInventoryItemDefinition.plug property will be populated with an instance of one of
///     these bad boys.
///     <para />
///     This gives information about when it can be inserted, what the plug's category is (and thus whether it is
///     compatible with a socket... see DestinySocketTypeDefinition for information about Plug Categories and socket
...
public sealed record DestinyItemPlugDefinition : IDeepEquatable<DestinyItemPlugDefinition>
{
    ...
    ///     <para />
    ///     The live data DestinyItemPlugComponent.insertFailIndexes will be an index into this array, so you can pull out the
    ///     failure strings appropriate for the user.
    /// </summary>
    ...
    ///     determine whether this plug can be inserted into the socket.
    /// </summary>
    ...
    /// <summary>
    ///     If inserting this plug requires materials, this is the DestinyMaterialRequirementSetDefinition for those
    ///     requirements.
    /// </summary>
    ...
    /// <summary>
    ///     It's not enough for the plug to be inserted. It has to be enabled as well. For it to be enabled, it may require
    ///     materials. This is the hash identifier for the DestinyMaterialRequirementSetDefinition for those requirements, if
    ///     there is one.
    /// </summary>
    ...
    ///     <para />
    ///     The live data DestinyItemPlugComponent.enableFailIndexes will be an index into this array, so you can pull out the
    ///     failure strings appropriate for the user.
    /// </summary>
    ...
    /// <summary>
    ///     Plugs can have arbitrary, UI-defined identifiers that the UI designers use to determine the style applied to plugs.
    ///     Unfortunately, we have neither a definitive list of these labels nor advance warning of when new labels might be
    ///     applied or how that relates to how they get rendered. If you want to, you can refer to known labels to change your
    ///     own styles: but know that new ones can be created arbitrarily, and we have no way of associating the labels with
    ///     any specific UI style guidance... you'll have to piece that together on your end. Or do what we do, and just show
    ///     plugs more generically, without specialized styles.
    /// </summary>
    ...
    /// <summary>
    ///     If the plug meets certain state requirements, it may have an alternative label applied to it. This is the
    ///     alternative label that will be applied in such a situation.
    /// </summary>
    [JsonPropertyName("alternateUiPlugLabel")]
    public string AlternateUiPlugLabel { get; init; }

    /// <summary>
    ///     The alternate plug of the plug: only applies when the item is in states that only the server can know about and
    ///     control, unfortunately. See AlternateUiPlugLabel for the related label info.
    /// </summary>
    ...
    /// <summary>
    ///     IF not null, this plug provides Energy capacity to the item in which it is socketed. In Armor 2.0 for example, is
    ///     implemented in a similar way to Masterworks, where visually it's a single area of the UI being clicked on to
