...
namespace NetBungieAPI.Services
{
    public class DatabaseComparer
    {
        ...
        public Dictionary<BungieLocales, Dictionary<DefinitionsEnum, Dictionary<uint, CompareResult>>> Results =
            new Dictionary<BungieLocales, Dictionary<DefinitionsEnum, Dictionary<uint, CompareResult>>>();
        ...
        //     foreach (var locale in locales)
        //     {
        //         var olderMobileWorldContentPathsLocalePath =
        //             Path.GetFileName(_older.MobileWorldContentPaths[locale.LocaleToString()]);
        //         using var olderManifestPipe = SQLiteDataPipe.CreatePipe(
        //             $"{repoPath}\\{_older.Version}\\MobileWorldContent\\{locale}\\{olderMobileWorldContentPathsLocalePath}");
        //
        //         var newerMobileWorldContentPathsLocalePath =
        //             Path.GetFileName(_newer.MobileWorldContentPaths[locale.LocaleToString()]);
        //         using var newerManifestPipe = SQLiteDataPipe.CreatePipe(
        //             $"{repoPath}\\{_newer.Version}\\MobileWorldContent\\{locale}\\{newerMobileWorldContentPathsLocalePath}");
        //
        //         foreach (var definition in definitions)
        //         {
        //             try
        ...
        public IEnumerable<CompareResult> GetNew(BungieLocales locale, DefinitionsEnum definition)
        {
            return Results[locale][definition].Where(x => x.Value.IsNew).Select(x => x.Value).ToList();
        }

        public List<DefinitionHashPointer<T>> GetNewAs<T>(BungieLocales locale, DefinitionsEnum definition)
        ...
        public List<DefinitionHashPointer<T>> GetUpdatedAs<T>(BungieLocales locale, DefinitionsEnum definition)
            where T : IDestinyDefinition
        {
        ...
        {
            Dictionary<DefinitionsEnum, List<CompareResult>> results =
                new Dictionary<DefinitionsEnum, List<CompareResult>>();
