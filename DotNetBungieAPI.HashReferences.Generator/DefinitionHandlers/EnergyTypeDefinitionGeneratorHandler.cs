...
public class EnergyTypeDefinitionGeneratorHandler : BaseDefinitionHandler, IDefinitionHandler
{
    public string FileSubName => nameof(DestinyEnergyTypeDefinition);

    public string ClassName => "EnergyTypes";
    ...
    public async Task Generate(IBungieClient bungieClient, TextWriter textWriter, StringBuilder stringBuilder, int indentation)
    {
        ...
        foreach (var definition in bungieClient.Repository.GetAll<DestinyEnergyTypeDefinition>())
        {
        ...
        {
            if (bungieClient.Repository.TryGetDestinyDefinition<DestinyEnergyTypeDefinition>(
