...
public class ArtifactDefinitionGeneratorHandler : BaseDefinitionHandler, IDefinitionHandler
{
    public string FileSubName => nameof(DestinyArtifactDefinition);

    public string ClassName => "Artifacts";
    ...
    public async Task Generate(IBungieClient bungieClient, TextWriter textWriter, StringBuilder stringBuilder, int indentation)
    {
        ...
        foreach (var definition in bungieClient.Repository.GetAll<DestinyArtifactDefinition>())
        {
        ...
        {
            if (bungieClient.Repository.TryGetDestinyDefinition<DestinyArtifactDefinition>(
