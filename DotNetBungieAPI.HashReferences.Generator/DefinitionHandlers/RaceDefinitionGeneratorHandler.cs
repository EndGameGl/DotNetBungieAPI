...
public class RaceDefinitionGeneratorHandler : BaseDefinitionHandler, IDefinitionHandler
{
    public string FileSubName => nameof(DestinyRaceDefinition);

    public string ClassName => "Races";
    ...
    public async Task Generate(IBungieClient bungieClient, TextWriter textWriter, StringBuilder stringBuilder, int indentation)
    {
        ...
        foreach (var definition in bungieClient.Repository.GetAll<DestinyRaceDefinition>())
        {
        ...
        {
            if (bungieClient.Repository.TryGetDestinyDefinition<DestinyRaceDefinition>(
