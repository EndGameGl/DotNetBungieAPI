...
public class ItemCategoryDefinitionGeneratorHandler : BaseDefinitionHandler, IDefinitionHandler
{
    public string FileSubName => nameof(DestinyItemCategoryDefinition);

    public string ClassName => "ItemCategories";
    ...
    public async Task Generate(IBungieClient bungieClient, TextWriter textWriter, StringBuilder stringBuilder, int indentation)
    {
        ...
        foreach (var definition in bungieClient.Repository.GetAll<DestinyItemCategoryDefinition>())
        {
        ...
        {
            if (bungieClient.Repository.TryGetDestinyDefinition<DestinyItemCategoryDefinition>(
